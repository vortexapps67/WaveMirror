...
function showToast(message) {
    if (window.WaveMirrorNative && window.WaveMirrorNative.showToast) {
        try {
            window.WaveMirrorNative.showToast(message);
        } catch (e) {}
    }
...
/* ================================================================
   DEDICATED APP ENGINE & TAB SYSTEM
   ================================================================ */
...
async function fetchCachedTMDB(url) {
...
function switchAppTab(tabName) {
    if (window.WaveMirrorNative && window.WaveMirrorNative.triggerHaptic) {
        try { window.WaveMirrorNative.triggerHaptic(); } catch (e) {}
    }

    currentAppTab = tabName;

    // Manage Views
    const views = document.querySelectorAll(".app-view, #partyView");
    views.forEach(v => {
...
// Initialize Apple Liquid Glass Live Dragging & Fluid Magnification Selector
...
function handleCategoryPill(category, pillElement) {
    if (window.WaveMirrorNative && window.WaveMirrorNative.triggerHaptic) {
        try { window.WaveMirrorNative.triggerHaptic(); } catch (e) {}
    }

    const pills = document.querySelectorAll(".app-category-bar .app-cat-pill");
...
async function initMoviesView() {
    ...
    const data = await fetchCachedTMDB(url);
    if (data && data.results) {
        const movies = parseTMDBItems(data.results, "movie");
        renderMoviesViewGrid(movies);
    }
}

async function filterMovieViewGenre(genre) {
    ...
    try {
        if (genre === "All") {
            const data = await fetchCachedTMDB(`${TMDB_BASE_URL}/trending/movie/week?api_key=${TMDB_API_KEY}`);
            items = data ? parseTMDBItems(data.results, "movie") : currentCatalog;
        } else {
            const genreId = GENRE_MAP[genre];
            if (genreId) {
                const data = await fetchCachedTMDB(`${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=${genreId}&sort_by=popularity.desc`);
                items = data ? parseTMDBItems(data.results, "movie") : [];
            }
        }
    } catch (e) {
...
function renderMoviesViewGrid(items) {
    const grid = document.getElementById("moviesViewGrid");
    if (!grid) return;
...
async function initSeriesView() {
    ...
    const data = await fetchCachedTMDB(url);
    if (data && data.results) {
        const series = parseTMDBItems(data.results, "tv");
        renderSeriesViewGrid(series);
    }
}

async function filterSeriesViewGenre(genre) {
    ...
    try {
        if (genre === "All") {
            const data = await fetchCachedTMDB(`${TMDB_BASE_URL}/trending/tv/week?api_key=${TMDB_API_KEY}`);
            items = data ? parseTMDBItems(data.results, "tv") : [];
        } else {
            const tvGenreMap = {
                "Drama": 18,
            ...
            const genreId = tvGenreMap[genre];
            if (genreId) {
                const data = await fetchCachedTMDB(`${TMDB_BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&with_genres=${genreId}&sort_by=popularity.desc`);
                items = data ? parseTMDBItems(data.results, "tv") : [];
            }
        }
    } catch (e) {
...
function renderSeriesViewGrid(items) {
    const grid = document.getElementById("seriesViewGrid");
    if (!grid) return;
    grid.innerHTML = items.map(show => `
        <div class="movie-card" role="button" tabindex="0" aria-label="Play ${String(show.title).replace(/"/g, '&quot;')}" onclick="openPlayerModal('${show.id}', 'tv')" onkeydown="handleCardKey(event, '${show.id}', 'tv')">
            ...
            <div class="movie-info">
                ...
                <div class="movie-subinfo">
                    <span>${show.year}</span>
                    <span style="color: var(--accent-gold); font-weight:700;">★ ${show.rating}</span>
                </div>
            </div>
        </div>
    `).join('');
...
// Discover & Universal Search Engine
let discoverSearchDebounce = null;
function handleDiscoverLiveSearch(val) {
...
async function applyDiscoverFilters() {
    const grid = document.getElementById("discoverViewGrid");
    const query = document.getElementById("discoverSearchInput")?.value.trim();
    const mediaType = document.getElementById("discoverTypeSelect")?.value || "all";
    const genre = document.getElementById("discoverGenreSelect")?.value || "all";
    const year = document.getElementById("discoverYearSelect")?.value || "all";
    ...
    try {
        if (query && query.length > 0) {
            const searchEndpoint = mediaType === "tv" ? "search/tv" : mediaType === "movie" ? "search/movie" : "search/multi";
            const url = `${TMDB_BASE_URL}/${searchEndpoint}?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(query)}`;
            const data = await fetchCachedTMDB(url);
            if (data && data.results) {
                results = parseTMDBItems(data.results.filter(r => r.media_type !== "person"), mediaType === "all" ? undefined : mediaType);
            }
        } else {
            const endpoint = mediaType === "tv" ? "discover/tv" : "discover/movie";
            let url = `${TMDB_BASE_URL}/${endpoint}?api_key=${TMDB_API_KEY}&sort_by=${sort}`;
            if (genre !== "all") url += `&with_genres=${genre}`;
            if (year !== "all") {
                if (year.endsWith("s")) {
                    const startYear = parseInt(year.replace("s", ""));
            ...
            const data = await fetchCachedTMDB(url);
            if (data && data.results) {
                results = parseTMDBItems(data.results, mediaType === "tv" ? "tv" : "movie");
            }
        }
    } catch (e) {
        console.warn("Discover filters query error:", e);
    }
    ...
    if (results.length === 0) {
        grid.innerHTML = `
            <div style="grid-column: 1/-1; text-align: center; padding: 4rem 1rem; color: var(--text-muted);">
                ...
                <h4 style="color: #fff; margin-bottom: 0.4rem;">No matching titles found</h4>
                <p style="font-size: 0.85rem;">Try adjusting your search terms, year, or genre filters.</p>
            </div>
        `;
        return;
    }
    ...
    grid.innerHTML = results.map(item => `
        <div class="movie-card" role="button" tabindex="0" aria-label="Play ${String(item.title).replace(/"/g, '&quot;')}" onclick="openPlayerModal('${item.id}', '${item.type || 'movie'}')" onkeydown="handleCardKey(event, '${item.id}', '${item.type || 'movie'}')">
            ...
            <div class="movie-info">
                ...
                <div class="movie-subinfo">
                    <span>${item.year || ''}</span>
                    <span style="color: var(--accent-gold); font-weight:700;">★ ${item.rating || '8.0'}</span>
                </div>
            </div>
        </div>
    `).join('');
...
function renderLibraryView() {
    const continueGrid = document.getElementById("libraryContinueGrid");
    const watchlistGrid = document.getElementById("libraryWatchlistGrid");
    const watchlistBadge = document.getElementById("libraryWatchlistBadge");
    const continueBadge = document.getElementById("libraryContinueBadge");

    const watchlist = getWatchlist();
    const continueList = getContinueWatching();

    if (watchlistBadge) watchlistBadge.innerText = watchlist.length;
    if (continueBadge) continueBadge.innerText = continueList.length;

    // 1. Render Continue Watching in Library
    if (continueGrid) {
        if (continueList.length === 0) {
            continueGrid.innerHTML = `
                <div style="grid-column: 1/-1; text-align: center; padding: 2.5rem 1rem; color: var(--text-muted); background: var(--surface-1); border-radius: var(--radius-md); border: 1px solid var(--border-glass);">
                    <p style="margin: 0; font-size: 0.9rem;">No active playback history. Start watching any movie or series to pick up where you left off!</p>
                </div>
            `;
        } else {
                ...
                <div class="continue-card" onclick="openPlayerModal('${item.id}', '${item.type || 'movie'}', true)">
                    ...
                    <div class="continue-content">
                        ...
                        <div class="continue-meta">
                            ${item.type === 'tv' ? `<span>S${item.season}:E${item.episode}</span> • ` : ''}
                            <span>${item.progressPercent || 0}% watched</span>
                        </div>
                        <div class="continue-progress-bar">
                            <div class="continue-progress-fill" style="width: ${item.progressPercent || 0}%;"></div>
                        </div>
                    </div>
                    <button class="continue-remove-btn" title="Remove" onclick="event.stopPropagation(); removeFromContinueWatching('${item.id}'); renderLibraryView();">✕</button>
                </div>
            `).join('');
        }
    }

    // 2. Render Watchlist in Library
    if (watchlistGrid) {
        if (watchlist.length === 0) {
            watchlistGrid.innerHTML = `
                <div style="grid-column: 1/-1; text-align: center; padding: 2.5rem 1rem; color: var(--text-muted); background: var(--surface-1); border-radius: var(--radius-md); border: 1px solid var(--border-glass);">
                    <p style="margin: 0; font-size: 0.9rem;">Your watchlist is empty. Tap "+ Watchlist" on any movie or series to bookmark it here.</p>
                </div>
            `;
        } else {
...
window.handleAndroidBack = function() {
    // 1. Close open modals or drawers first
    const openModals = document.querySelectorAll(".modal-backdrop.active, #drawerBackdrop.active, .mobile-search-modal.active");
    ...
    // 2. If on a tab other than home, navigate back to home
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (window.WaveMirrorNative || navigator.userAgent.includes("WaveMirrorApp")) {
        ...
        if (logoBadge) {
            logoBadge.innerText = "APP EDITION";
