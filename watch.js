...
/* ---------------- 📥 Direct File & Stream Downloader Engine ---------------- */
async function openDownloadModal() {
    ...
    const filesList = document.getElementById("directFilesList");
    const statusPill = document.getElementById("downloadStatusPill");
    const streamBtn1 = document.getElementById("streamDirectBtn1");
    const streamBtn2 = document.getElementById("streamDirectBtn2");
    ...
    if (modalSub) {
        modalSub.innerText = currentType === "tv"
            ? `Season ${s}, Episode ${e} • 1-Click direct file & magnet downloads`
            : `Direct offline video files & magnet links (1080p, 720p, 4K)`;
    }

    // Set stream gateway URLs
    if (streamBtn1) {
        streamBtn1.href = currentType === "tv" 
            ? `https://vidlink.pro/tv/${currentId}/${s}/${e}` 
            : `https://vidlink.pro/movie/${currentId}`;
    }
    if (streamBtn2) {
        streamBtn2.href = currentType === "tv" 
    ...
    if (filesList && statusPill) {
        filesList.innerHTML = `<div style="font-size: var(--fs-2xs); color: var(--text-muted); padding: 0.5rem; text-align: center;">Searching direct mirrors...</div>`;
        ...
        try {
            if (currentType === "movie") {
                ...
                if (matched && matched.torrents && matched.torrents.length > 0) {
                    statusPill.innerText = "✓ Direct Mirrors Available";
                    statusPill.style.color = "var(--accent-emerald)";

                    filesList.innerHTML = matched.torrents.map(t => {
                        ...
                        return `
                            <div class="download-option-card" style="padding: 0.65rem 0.85rem;">
                                <div style="display: flex; align-items: center; gap: 0.6rem;">
                                    <span style="font-size: 1.2rem;">🎬</span>
                                    <div>
                                        <div style="font-weight: 600; font-size: var(--fs-xs); color: #fff;">${quality} ${type} (${size})</div>
                                        <div style="font-size: var(--fs-2xs); color: var(--text-muted);">Direct file download • High speed</div>
                                    </div>
                                </div>
                                <div style="display: flex; gap: 0.35rem; align-items: center;">
                                    <a href="${downloadUrl}" download class="btn-primary" style="padding: 0.35rem 0.7rem; font-size: 11px; text-decoration: none;">
                                        ⬇️ Download
                                    </a>
                                    <a href="${magnetLink}" class="btn-secondary" style="padding: 0.35rem 0.6rem; font-size: 11px; text-decoration: none;">
                                        🧲 Magnet
                                    </a>
                                </div>
                            </div>
                    ...
                    renderFallbackDownload(movieTitle, year, s, e, filesList, statusPill);
                }
            } else {
                renderFallbackDownload(movieTitle, year, s, e, filesList, statusPill);
            }
        } catch (err) {
            console.warn("[Download] Resolver error:", err);
            renderFallbackDownload(movieTitle, year, s, e, filesList, statusPill);
        }
    }
...
function renderFallbackDownload(title, year, s, e, container, statusElem) {
    if (statusElem) {
        statusElem.innerText = "1-Click Direct Ready";
    ...
    container.innerHTML = `
        <div class="download-option-card" style="padding: 0.65rem 0.85rem;">
            <div style="display: flex; align-items: center; gap: 0.6rem;">
                <span style="font-size: 1.2rem;">🧲</span>
                <div>
                    <div style="font-weight: 600; font-size: var(--fs-xs); color: #fff;">1-Click Direct Torrent Download</div>
                    <div style="font-size: var(--fs-2xs); color: var(--text-muted);">${query}</div>
                </div>
            </div>
            <a href="https://1337x.to/search/${encodeURIComponent(query)}/1/" target="_blank" rel="noopener noreferrer" class="btn-primary" style="padding: 0.35rem 0.75rem; font-size: 11px; text-decoration: none;">
                ⬇️ Find File
            </a>
        </div>
    `;
